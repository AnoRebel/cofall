...
(function () {
  ...
  function FileBufferReaderHelper() {
    ...
    fbrHelper.readAsArrayBuffer = function (fbr, options) {
      ...
      function processChunk(chunk) {
        ...
        // for huge files
        if (
          (chunk.maxChunks > 200 && chunk.currentPosition == 200) &&
          earlyCallback
        ) {
          earlyCallback(chunk.uuid);
          earlyCallback = null;
    ...
    function fileReaderWrapper(options, callback) {
      ...
      var chunksPerSlice = Math.floor(
        Math.min(100000000, cacheSize) / chunkSize,
      );
        ...
        fbReceiver.chunksWaiters[chunk.uuid] = function () {
          function looper() {
            ...
            if (
              ...
              !fbReceiver.chunks[chunk.uuid][chunk.currentPosition]
            ) {
              callback(chunk);
  ...
  var binary_dump = function (view, start, length) {
    ...
    for (i = 0; i < length; i++) {
      ...
      table[index][i % ROW_LENGTH] = code < 16
        ? "0" + code.toString(16)
        : code.toString(16);
    }
  ...
  var find_type = function (obj) {
    ...
    if (obj === undefined) {
      ...
      if (
        ...
        Types[const_name.toUpperCase()] !== undefined
      ) {
        // return type by .constructor.name if possible
        type = Types[const_name.toUpperCase()];
      } else if (
        const_name_reflection !== undefined &&
        Types[const_name_reflection.toUpperCase()] !== undefined
      ) {
  ...
  var serialize = function (obj, callback) {
    ...
    unit = Length[type] === undefined || Length[type] === null
      ? 0
