...
function RecordRTC(mediaStream, config) {
    ...
    function processInWebWorker(_function) {
      try {
        var blob = URL.createObjectURL(
          new Blob([
            _function.toString(),
            "this.onmessage =  function (eee) {" + _function.name +
            "(eee.data);}",
          ], {
  ...
  var returnObject = {
    ...
    reset: function () {
      ...
      if (
        mediaRecorder && typeof mediaRecorder.clearRecordedData === "function"
      ) {
        mediaRecorder.clearRecordedData();
      }
      mediaRecorder = null;
...
RecordRTC.getFromDisk = function (type, callback) {
  ...
  console.log(
    "Getting recorded " + (type === "all" ? "blobs" : type + " blob ") +
      " from disk!",
  );
...
function MRecordRTC(mediaStream) {
  ...
  this.startRecording = function () {
    ...
    if (
      typeof mediaType.audio !== "function" && isMediaRecorderCompatible() &&
      !getTracks(mediaStream, "audio").length
    ) {
    ...
    if (
      typeof mediaType.video !== "function" && isMediaRecorderCompatible() &&
      !getTracks(mediaStream, "video").length
    ) {
    ...
    if (
      typeof mediaType.gif !== "function" && isMediaRecorderCompatible() &&
      !getTracks(mediaStream, "video").length
    ) {
      mediaType.gif = false;
    }
    ...
    if (!!mediaType.video) {
      ...
      if (
        isMediaRecorderCompatible() && !!mediaType.audio &&
    ...
    if (!!mediaType.audio && !!mediaType.video) {
      ...
      if (mediaType.audio instanceof StereoAudioRecorder && !!mediaType.video) {
        ...
        mediaType.audio !== true && mediaType.video !== true &&
...
(function (that) {
  ...
  if (
    ...
    global.console.error = global.console.log = global.console.log ||
      function () {
  ...
  if (typeof document === "undefined") {
    ...
    document.createElement = document.captureStream = document
...
var isChrome = (!isOpera && !isEdge && !!navigator.webkitGetUserMedia) ||
  isElectron() || navigator.userAgent.toLowerCase().indexOf("chrome/") !== -1;
...
if (
  typeof MediaStream === "undefined" && typeof webkitMediaStream !== "undefined"
) {
  ...
  this.record = function () {
    ...
    if (
      ...
      if (!MediaRecorder.isTypeSupported(recorderHints.mimeType)) {
        ...
        recorderHints.mimeType = config.type === "audio"
          ? "audio/webm"
          : "video/webm";
      }
    }
    ...
    // old hack?
    if (
      recorderHints.mimeType && !MediaRecorder.isTypeSupported &&
    ...
    mediaRecorder.onerror = function (error) {
      ...
      (function (looper) {
        if (
          !self.manuallyStopped && mediaRecorder &&
      ...
      if (
        mediaRecorder.state !== "inactive" && mediaRecorder.state !== "stopped"
      ) {
        mediaRecorder.stop();
      }
    };
...
function StereoAudioRecorder(mediaStream, config) {
  ...
  if (
    ...
    RecordRTC.Storage.AudioContextConstructor = new RecordRTC.Storage
      .AudioContext();
  }
  ...
  var bufferSize = typeof config.bufferSize === "undefined"
    ? 4096
    : config.bufferSize;
   ...
   * });
   */
  var sampleRate = typeof config.sampleRate !== "undefined"
    ? config.sampleRate
  ...
  function looper() {
    if (
      !recording || typeof config.ondataavailable !== "function" ||
    ...
    if (intervalsBasedBuffers.left.length) {
      mergeLeftRightBuffers({
        ...
        rightBuffers: numberOfAudioChannels === 1
          ? []
...
function WhammyRecorder(mediaStream, config) {
  ...
  function dropBlackFrames(
    ...
    var pixTolerance = _pixTolerance && _pixTolerance >= 0 && _pixTolerance <= 1
      ? _pixTolerance
      : 0;
    var frameTolerance =
...
var Whammy = (function () {
  ...
  function whammyInWebWorker(frames) {
    function ArrayToWebM(frames) {
      ...
      while (frameNumber < frames.length) {
        ...
        do {
          ...
          frameNumber < frames.length && clusterDuration < clusterMaxDuration
        );
    ...
    function getStrLength(string, offset) {
      return parseInt(
        string.substr(offset + 4, 4).split("").map(function (i) {
...
var DiskStorage = {
  ...
  init: function () {
    ...
    if (
      typeof indexedDB === "undefined" || typeof indexedDB.open === "undefined"
    ) {
    ...
    function putInDB() {
      ...
      if (self.videoBlob) {
        transaction.objectStore(self.dataStoreName).put(
          self.videoBlob,
          "videoBlob",
        );
      }
  ...
  this.appendStreams = function (streams) {
    ...
    streams.forEach(function (stream) {
      ...
      if (
        ...
        var audioSource = self.audioContext.createMediaStreamSource(stream);
        self.audioDestination = self.audioContext
